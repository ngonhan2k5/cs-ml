...
class TopRateMovieForUser:
	...
	def train_data(self):
		...
		reader = Reader()
		data = Dataset.load_from_df(self.data_frame[['userId', 'movieId', 'rating']], reader)
	...
	def get_top_ten_rate_of_user(self, user_id):
		...
		if user_id in self.data_frame['userId'].tolist():
			movie_id_list = list(dict.fromkeys(self.data_frame['movieId'].tolist()))
			movie_est_rate = []

			for movie_id in movie_id_list:
				predict = self.model.predict(user_id, movie_id)
				movie_est_rate.append({'movie_id' : movie_id, 'est' : predict.est})
